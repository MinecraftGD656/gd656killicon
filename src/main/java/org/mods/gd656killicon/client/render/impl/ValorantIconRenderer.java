...
public class ValorantIconRenderer implements IHudRenderer {
    ...
    public void renderAt(GuiGraphics guiGraphics, float partialTick, float centerX, float centerY) {
        ...
        float iconWidth = ICON_BASE_SIZE * configIconScale * resolveFrameRatio("icon", "texture_frame_width_ratio");
        float iconHeight = ICON_BASE_SIZE * configIconScale * resolveFrameRatio("icon", "texture_frame_height_ratio");
        float barBaseSize = BAR_BASE_SIZE * configBarScale * (gaiaSkin ? GAIA_BAR_SCALE : 1.0f);
        float barWidth = barBaseSize * resolveFrameRatio("bar", "texture_frame_width_ratio");
        float barHeight = barBaseSize * resolveFrameRatio("bar", "texture_frame_height_ratio");
        ...
        try {
            ...
            drawColorizableTexture(guiGraphics, iconTexturePath, iconWidth, iconHeight, alpha, 0xFFFFFF, false, accentTintEnabled, accentColor);
            if (flash.alpha() > 0.0f) {
                drawColorizableTexture(guiGraphics, iconTexturePath, iconWidth, iconHeight, alpha * flash.alpha(), flash.color(), true, accentTintEnabled, accentColor);
            }
            RenderSystem.defaultBlendFunc();
    ...
    private void renderBarRing(
        ...
        for (float layoutAngle : layoutAngles) {
            ...
            drawColorizableTexture(guiGraphics, barTexturePath, barWidth, barHeight, alpha, 0xFFFFFF, false, accentTintEnabled, accentColor);
            RenderSystem.defaultBlendFunc();
            guiGraphics.pose().popPose();
        }
    }
